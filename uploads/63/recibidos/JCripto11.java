...
/////////////////////////////////////////////////////////////
  private BigInteger getPrivCode(DataInputStream key){//extrae la clave privada de un archivo .pri
  	...
  	try{//cabecera
	    rango=BigInteger.ZERO;
	    rango=rango.setBit(key.readInt());//cantidad de dígitos
  ...
  private BigInteger getPubCode(DataInputStream key){//extrae la clave pública de un archivo .pub
	  byte[] b;
	  BigInteger rango;

  	try{//cabecera
	    b=new byte[key.readInt()];//tamaño de x2
  ...
  private void cifrar(boolean cifrar, String fnamein, String fnameout, String fkey){
	  ...
	  try{
	    ...
	    if (cifrar){//cifrar
    		...
    		out.writeBoolean(k);
    		if (k) convert(getPubCode(key), true, in, out);//cifrado con clave pública
    		...
    		if (in.readBoolean()==(!k)){
  		    ...
  		    if (k) convert(getPubCode(key), false, in, out);//descifrado con clave pública
